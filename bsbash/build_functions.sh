...
C_COMPILER="/usr/bin/gcc-12.2"
CXX_COMPILER="/usr/bin/gcc-12.2"
AR_COMMAND="ar"
RANLIB="ranlib"
C_FLAGS=
ROOT_DIR=$(pwd)
...
COMPILE_EXECUTABLE () {
	...
	if [ -n "$FORCE_COMPILATION" ]; then
		...
		local COMPILER_COMMAND="/usr/bin/gcc-12.2 $MY_C_FLAGS $SOURCES ${STATIC_DEPS[*]}  -o $BUILD_DIR/${EXECUTABLE_NAME} ${EXTRA_LIBRARY_PATH[*]} ${DYNAMIC_DEPS[*]} -Wl,-rpath=$LIBRARY_OUTPUT_DIRECTORY"

...
GET_CELL_MODEL_DATA(init_cell_model_data) {
    ...
    if(get_neq)
        cell_model->number_of_ode_equations = NEQ + 3;
}
...
SET_ODE_INITIAL_CONDITIONS_CPU(set_model_initial_conditions_cpu) {
    ...
    solver->sv = (real*) malloc((NEQ + 3) * num_cells * sizeof(real));

    solver->ode_time_new = (real*) calloc(num_cells, sizeof(real));

    OMP(parallel for)
    for(uint32_t i = 0; i < num_cells; i++) {
        real *sv = &solver->sv[i * (NEQ + 3)];
        ...
        sv[12] = 0;     // number activation times
        sv[13] = 0.0f;  // last activation time
        sv[14] = -86.2f;  // last v
    }
}
...
SOLVE_MODEL_ODES(solve_model_odes_cpu) {
    ...
    for(i = 0; i < num_cells_to_solve; i++) {
        ...
        for(int j = 0; j < num_steps; ++j) {
            if(mapping[i] == 0)         // ENDO
                solve_model_ode_cpu(dt, sv + (sv_id * (NEQ + 3)), stim_currents[i], mapping[i], sv_id, current_t);
...
void solve_model_ode_cpu(real dt, real *sv, real stim_current, int type_cell, uint32_t sv_id, real time) {
    assert(sv);

    real rY[NEQ + 3], rDY[NEQ + 3];
    ...
    if((sv[14] < activation_threshold) && (sv[0] >= activation_threshold)) {

        if(rY[12] == 0) {
            sv[12] = rY[12] + 1;        // num act times
            sv[13] = time;              // last act time
        } else { // This is to avoid spikes in the middle of an Action Potential
            if(time - rY[13] > time_threshold) {
                sv[12] = rY[12] + 1;        // num act times
                sv[13] = time;              // last act time
            }
        }
...
void RHS_cpu(const real *sv, real *rDY_, real stim_current, real dt, int type_cell) {
    ...
    INaCa = knaca * (1. / (KmNai * KmNai * KmNai + Nao * Nao * Nao)) * (1. / (KmCa + Cao)) * (1. / (1 + ksat * exp((n - 1) * svolt * F / (R * T)))) *
    ...
    (sItot) = IKr + IKs + IK1 + Ito + INa + IbNa + ICaL + IbCa + INaK + INaCa + IpCa + IpK + IKatp + stim_current;

...
#include "tt3_mixed_with_activations_times.h"
#include <stddef.h>
...
uint32_t *mapping = NULL;
    ...
    solve_gpu<<<GRID, BLOCK_SIZE>>>(dt, sv, stims_currents_device, cells_to_solve_device, num_cells_to_solve, num_steps, fibrosis_device, extra_parameters_device,mapping_device, current_t);
...
__global__ void kernel_set_model_inital_conditions(real *sv, real*IC, int num_volumes)
{
    ...
    if(threadID < num_volumes) {
        ...
        *((real *) ((char *) sv + pitch * 12) + threadID) = 0; //number activation times
        *((real *) ((char *) sv + pitch * 13) + threadID) = 0.0f; //last activation time
        *((real *) ((char *) sv + pitch * 14) + threadID) = INITIAL_V; //last v
    }
}
...
__global__ void solve_gpu(real dt, real *sv, real* stim_currents,
                          ...
                          int num_steps, real *fibrosis, real *extra_parameters,uint32_t *mapping, real time)
{
    int threadID = blockDim.x * blockIdx.x + threadIdx.x;
    int sv_id;
    ...
    if(threadID < num_cells_to_solve) { 
        ...
        *((real*)((char*)sv) + sv_id) = dt*rDY[0] + *((real*)((char*)sv) + sv_id);
        ...
        // Update number activation times
        real time_threshold = 10.0;
        real activation_threshold = -30.0;

        if((*((real*)((char*)sv + pitch * 14) + sv_id) < activation_threshold) && (*((real*)((char*)sv + pitch * 0) + sv_id) >= activation_threshold)) {

            if(*((real*)((char*)sv + pitch * 12) + sv_id) == 0) {
                *((real*)((char*)sv + pitch * 12) + sv_id) += 1;        // num act times
                *((real*)((char*)sv + pitch * 13) + sv_id) = time;      // last act time
            } else { // This is to avoid spikes in the middle of an Action Potential
                if(time - *((real*)((char*)sv + pitch * 13) + sv_id) > time_threshold) {
                    *((real*)((char*)sv + pitch * 12) + sv_id) += 1;        // num act times
                    *((real*)((char*)sv + pitch * 13) + sv_id) = time;      // last act time
                }
            }
        ...
        *((real*)((char*)sv + pitch * 14) + sv_id) = *((real*)((char*)sv + pitch * 0) + sv_id);

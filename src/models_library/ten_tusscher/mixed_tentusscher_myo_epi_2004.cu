...
__global__ void kernel_set_model_inital_conditions(real *sv, uint32_t *mapping, int num_volumes) 
{
    ...
    if (threadID < num_volumes) 
    {
        ...
        // Initial conditions for TenTusscher 2004 endo
        ...
        // Initial conditions for TenTusscher 2004 myo
...
// Initial conditions for TenTusscher 2004 epi
                          ...
                          int num_steps)
{
    ...
    if(threadID < num_cells_to_solve) 
    {
		...
		for (int n = 0; n < num_steps; ++n) {
			...
			if (mapping[sv_id] == 0) {
				RHS_gpu_endo(sv, rDY, stim_currents[threadID], sv_id, dt);
			}
...
inline __device__ void RHS_gpu_endo (real *sv_, real *rDY_, real stim_current, int threadID_, real dt)
{
    // State variables
     real svolt = *((real*)((char*)sv_ + pitch * 0) + threadID_);
     real sm    = *((real*)((char*)sv_ + pitch * 1) + threadID_);
